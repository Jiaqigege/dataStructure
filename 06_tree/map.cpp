...
public:
    ...
    Value &getOrDefault(K key)
    {
        if (this->root == nullptr) {
            ...
            return this->getNodeOrProvide(this->root, key, [&key]() { return Node::from(key); })
                ->value;
        }
    }
    ...
    Value &getOrInsert(K key, V value)
    {
        if (this->root == nullptr) {
            ...
            NodePtr node =
                getNodeOrProvide(this->root, key, [&]() { return Node::from(key, value); });
            return node->value;
        }
    }
    ...
    {
        this->inorderTraversal([&](ConstNodePtr node) { action(node->key, node->value); });
    }
    ...
    {
        EntryList entryList;
        this->inorderTraversal([&](ConstNodePtr node) { entryList.push_back(node->entry()); });
        return entryList;
    }
